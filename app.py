...
if not st.session_state.agreed:
    ...
    if st.checkbox("I have read and agree to the Terms of Use."):
        if st.button("Accept & Enter Pro Analyzer"):
            ...
            st.rerun()
        st.markdown("""<script>setTimeout(function() {var c = window.parent.document.querySelector('[data-testid="stAppViewContainer"]'); if (c) {c.scrollTo({top: c.scrollHeight, behavior: 'smooth'});}}, 300);</script>""", unsafe_allow_html=True)
    st.stop()

# --- SCROLL TO TOP FIX ---
if st.session_state.get('scroll_to_top'):
    st.markdown("""<script>var body = window.parent.document.querySelector(".main"); if (body) { body.scrollTop = 0; }</script>""", unsafe_allow_html=True)
...
with tab_anal:
    ...
    with c_client: client_name = st.text_input("Prepared For", placeholder="e.g. Acme Properties LLC")
    with c_addr: prop_address = st.text_input("Property Address", placeholder="e.g. 123 Main St, Rome, GA")
...
with tab_iq:
    ...
    st.write("Expert Knowledge Base...")