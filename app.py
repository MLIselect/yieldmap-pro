...
# --- 2. VISUAL UPGRADE: CUSTOM CSS (Sticky Header + Navbar) ---
st.markdown("""
    <style>
    /* 1. GLOBAL FONT RESET */
    html, body, [class*="css"] {
    ...
    /* 2. PUSH CONTENT DOWN */
    /* This ensures content isn't hidden behind the fixed header */
    .block-container {
        padding-top: 7rem; 
        padding-bottom: 5rem;
    }
    ...
    .brand-container {
        ...
        margin-right: 40px; 
        /* Ensure brand stays on top of any absolute positioning */
        z-index: 100003; 
    }
    ...
    /* 7. NAVIGATION BAR STYLING (Moving st.radio into Header) */
    /* This targets the radio widget container */
    div[data-testid="stRadio"] {
        position: fixed;
        top: 20px;
        left: 350px; /* Position to right of logo */
        z-index: 100002;
    ...
    div[role="radiogroup"] label[data-checked="true"] {
        background-color: rgba(255, 255, 255, 0.15) !important; /* White pill bg */
    }
    ...
    /* SETTINGS BUTTON (Right Side - Floating) */
...
if page == "Pro Analyzer":
    ...
    if not st.session_state.portfolio:
        ...
        st.dataframe(comp_df.style.format({"Price": "${:,.0f}", "Rent": "${:,.0f}", "CoC": "{:.1f}%", "Cashflow": "${:,.0f}"}).apply(highlight_max, subset=['CoC', 'Cashflow']), use_container_width=True)
